...
train_pgo() {
    ...
    for f in /build/samples/*.mkv; do
        ...
        i=$(echo "$json" | grep 'input_i' | tail -1 | sed 's/.*: "\([^"]*\)".*/\1/')
        tp=$(echo "$json" | grep 'input_tp' | tail -1 | sed 's/.*: "\([^"]*\)".*/\1/')
        lra=$(echo "$json" | grep 'input_lra' | tail -1 | sed 's/.*: "\([^"]*\)".*/\1/')
        thresh=$(echo "$json" | grep 'input_thresh' | tail -1 | sed 's/.*: "\([^"]*\)".*/\1/')
        offset=$(echo "$json" | grep 'target_offset' | tail -1 | sed 's/.*: "\([^"]*\)".*/\1/')
